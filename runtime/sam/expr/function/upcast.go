...
// add to tests
// echo "1::(int64|string)::((int64|string)|(int64|bool)) true" | go run ./cmd/super -c "fuse" -
// (fuse shouldn't have a fusion type here for single value)
...
func (u *Upcast) toContainer(b *scode.Builder, elemType super.Type, bytes scode.Bytes, toElemType super.Type) bool {
	...
	for it := bytes.Iter(); !it.Done(); {
		if ok := u.build(b, elemType, it.Next(), toElemType); !ok {
			return false
		}
	}
	b.EndContainer()
...
// XXX need to fix for named types
func upcastUnionTag(types []super.Type, typ super.Type) int {
	if named, ok := typ.(*super.TypeNamed); ok {
		return slices.IndexFunc(types, func(t super.Type) bool {
			if t, ok := t.(*super.TypeNamed); ok {
				return t.Name == named.Name
			}
			return false
		})
	}
